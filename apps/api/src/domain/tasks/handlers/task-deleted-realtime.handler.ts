import { Injectable } from '@nestjs/common';
import { OnEvent } from '@nestjs/event-emitter';
import { RealtimePublisher } from '../../realtime/realtime.publisher';
import { REALTIME_EVENT } from '../../realtime/realtime.events';
import { TASK_DOMAIN_EVENT_NAME } from '../events/task-domain-event-names';
import type { TaskDeletedEvent } from '../events/task-domain.events';

@Injectable()
export class TaskDeletedRealtimeHandler {
  constructor(private readonly realtimePublisher: RealtimePublisher) {}

  @OnEvent(TASK_DOMAIN_EVENT_NAME.TaskDeleted)
  handle(event: TaskDeletedEvent): void {
    const payload = {
      taskId: event.taskId,
      projectId: event.projectId,
      updatedById: event.updatedById,
      changedFields: ['deleted'] as const,
    };

    this.realtimePublisher.toProject(
      event.projectId,
      REALTIME_EVENT.taskDeleted,
      payload,
    );
    this.realtimePublisher.toTask(
      event.taskId,
      REALTIME_EVENT.taskDeleted,
      payload,
    );
  }
}
