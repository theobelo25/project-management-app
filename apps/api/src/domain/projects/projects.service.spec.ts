...
describe('ProjectsService', () => {
  ...
  let queries: jest.Mocked<
    Pick<ProjectsQueriesService, 'findManyForUser' | 'findById' | 'findDetailById'>
  >;
  ...
  it('mostly just forwards to commands / queries', async () => {
    ...
    commands.create.mockResolvedValue({} as any);
    queries.findManyForUser.mockResolvedValue({} as any);
    queries.findById.mockResolvedValue({} as any);
    queries.findDetailById.mockResolvedValue({} as any);
    commands.update.mockResolvedValue({} as any);
    commands.archive.mockResolvedValue({} as any);
    ...
    await service.findById('p1', me, {} as any);
    ...
    expect(commands.update).toHaveBeenCalledWith('p1', me, { name: 'n' }, undefined);
  ...
  it('unarchive still goes through commands', async () => {
    commands.unarchive.mockResolvedValue({} as any);
